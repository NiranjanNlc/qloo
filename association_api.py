...
class ComboOffer(BaseModel):
    ...
    product_names: List[str] = Field(
        default_factory=list, description="Names of products in the combo"
    )
    confidence_score: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence score for the combo"
    )
    support: float = Field(
        ..., ge=0.0, le=1.0, description="Support metric from association rules"
    )
    lift: float = Field(..., gt=0.0, description="Lift metric from association rules")
    expected_discount_percent: Optional[float] = Field(
        None, ge=0.0, le=100.0, description="Expected discount percentage"
    )
    category_mix: Optional[List[str]] = Field(
        None, description="Categories included in combo"
    )
    estimated_revenue: Optional[float] = Field(
        None, ge=0.0, description="Estimated revenue from combo"
    )
...
class CombosResponse(BaseModel):
    ...
    filters_applied: Dict[str, Any] = Field(
        default_factory=dict, description="Filters applied to the results"
    )
...
class ProductAssociation(BaseModel):
    ...
    association_strength: float = Field(
        ..., ge=0.0, le=1.0, description="Strength of association"
    )
...
class AssociationsResponse(BaseModel):
    ...
    associations: List[ProductAssociation] = Field(
        ..., description="List of product associations"
    )
    total_associations: int = Field(
        ..., ge=0, description="Total number of associations found"
    )
    ...
    if _combo_generator is None:
        try:
            ...
            _combo_generator = ComboGenerator(
                min_confidence=0.6, min_support=0.01, price_api=price_api
            )
            logger.info("Combo generator initialized")
        except Exception as e:
            logger.error(f"Failed to initialize combo generator: {e}")
            raise HTTPException(
                status_code=500, detail="Failed to initialize combo generator"
            )
...
def get_association_engine():
    ...
    if _association_engine is None:
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to initialize association engine"
            )
...
async def get_combos(
    limit: int = Query(
        50, ge=1, le=100, description="Maximum number of combos to return"
    ),
    ...
    sort_by: str = Query(
        "confidence", description="Sort field (confidence, lift, support, created_at)"
    ),
    ...
    try:
        ...
        for combo in filtered_combos:
            # Get product names
            product_names = [
                products.get(pid, {}).get("name", f"Product {pid}")
                for pid in combo.products
            ]

            # Calculate estimated revenue (mock calculation)
            base_revenue = sum(
                products.get(pid, {}).get("price", 5.0) for pid in combo.products
            )
            estimated_revenue = (
                base_revenue * combo.confidence_score * 0.8
            )  # Assume 80% of base revenue
        ...
        logger.info(
            f"Returned {len(combo_offers)} combos with filters: {response.filters_applied}"
        )
        return response
...
async def get_product_associations(
    product_id: int,
    limit: int = Query(
        10, ge=1, le=50, description="Maximum number of associations to return"
    ),
    min_strength: float = Query(
        0.1, ge=0.0, le=1.0, description="Minimum association strength"
    ),
):
    """
    ...
    try:
        ...
        logger.info(
            f"Returned {len(associations)} associations for product {product_id}"
        )
        return response
...
async def get_metrics(
    ...
    try:
        ...
        avg_confidence = (
            statistics.mean([combo.confidence_score for combo in combos])
            if combos
            else 0
        )
        avg_lift = statistics.mean([combo.lift for combo in combos]) if combos else 0
        avg_discount = (
            statistics.mean([combo.expected_discount_percent for combo in combos])
            if combos
            else 0
        )
        high_confidence_rules = len([c for c in combos if c.confidence_score >= 0.8])
        total_revenue_potential = sum(
            [combo.confidence_score * 100 for combo in combos]
        )  # Mock revenue potential
        ...
        business_metrics = {
            ...
            "combo_success_rate": round(
                (high_confidence_rules / total_combos * 100) if total_combos > 0 else 0,
                1,
            ),
        }
        ...
        kpis = {
            ...
            "api_performance": {
                ...
                "status": (
                    "good"
                    if performance_metrics["api_response_time_ms"] <= 200
                    else "warning"
                ),
                "unit": "ms",
            },
        ...
        if include_trends:
            for kpi_name, kpi_data in kpis.items():
                if kpi_data["previous"] is not None:
                    ...
                    change_percent = (
                        (change / kpi_data["previous"] * 100)
                        if kpi_data["previous"] != 0
                        else 0
                    )
        ...
        if format == "summary":
            ...
            return MetricsResponse(
                ...
                period=period,
                kpis={
                    k: {"current": v["current"], "status": v["status"]}
                    for k, v in kpis.items()
                },
                performance={
                    "response_time": performance_metrics["api_response_time_ms"],
                    "success_rate": performance_metrics["success_rate"],
                },
                business={
                    "total_combos": business_metrics["total_combos_generated"],
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "association_api:app", host="0.0.0.0", port=8000, reload=True, log_level="info"
    )
