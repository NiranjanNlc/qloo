...
def test_api_connection():
    ...
    try:
        ...
        print(
            f"{'✅' if is_healthy else '❌'} Health check: {'PASSED' if is_healthy else 'FAILED'}"
        )
    ...
    test_cases = [
        {
            "product_id": "apple",
            "type": "product",
            "description": "Product recommendations for apple",
        },
        {
            "product_id": "milk",
            "type": "food",
            "description": "Food recommendations for milk",
        },
        {
            "product_id": "bread",
            "type": "item",
            "description": "Item recommendations for bread",
        },
    ]
    ...
    for case in test_cases:
        try:
            ...
            recommendations = client.get_product_recommendations(
                case["product_id"], limit=5, recommendation_type=case["type"]
            )
...
def test_product_associations(client):
    ...
    test_cases = [
        ...
        },
        {
            ...
            "type": "food",
            "description": "Food associations",
        },
        {
            "products": ["eggs", "cheese"],
            "type": "item",
            "description": "Item associations",
        },
    ]
    ...
    for case in test_cases:
        try:
            ...
            associations = client.get_product_associations(
                case["products"], association_type=case["type"]
            )
            ...
            if associations:
                ...
                for product_id, assocs in list(associations.items())[:2]:
                    ...
                    if assocs:
                        ...
                        print(
                            f"      → {first_assoc.get('associated_product_id', 'Unknown')}"
                        )
    ...
    test_cases = [
        {
            "category": "beverages",
            "type": "category",
            "description": "Category insights for beverages",
        },
        {
            "category": "dairy",
            "type": "food_category",
            "description": "Food category insights for dairy",
        },
        {
            "category": "produce",
            "type": "section",
            "description": "Section insights for produce",
        },
    ]
    ...
    for case in test_cases:
        try:
            ...
            insights = client.get_category_insights(
                case["category"], insight_type=case["type"]
            )
            ...
            if insights:
                ...
                if "seasonal_trends" in insights:
                    print(
                        f"   Seasonal data: {len(insights['seasonal_trends'])} seasons"
                    )
...
def test_different_parameter_combinations(client):
    ...
    for type_value in type_values:
        try:
            ...
            recommendations = client.get_product_recommendations(
                test_product, limit=3, recommendation_type=type_value
            )
...
def test_api_diagnostics(client):
    ...
    for endpoint, result in endpoint_results.items():
        ...
        print(
            f"{status_icon} {endpoint}: {result.get('status_code', result['status'])}"
        )
    ...
    for combo_name, result in search_results.items():
        ...
        print(
            f"{status_icon} {params_str}: {result.get('status_code', result['status'])}"
        )
        if result["status"] == "success":
            print(f"   🎉 SUCCESS! Found working parameters!")
            print(
                f"   📦 Response keys: {list(result['data'].keys()) if isinstance(result['data'], dict) else 'Non-dict response'}"
            )
            return True
