...
def test_recommendations_with_type_parameter(base_url: str, api_key: str):
    ...
    for type_value in type_values:
        try:
            params = {"input": "apple", "type": type_value, "limit": 3}
            ...
            if response.status_code < 400:
                try:
                    ...
                    if "recommendations" in data:
                        ...
                        print(
                            f"   🎯 Found data with {len(data['data']) if isinstance(data['data'], list) else 'N/A'} items"
                        )
                    else:
                ...
                try:
                    ...
                    print(
                        f"   💡 Error: {error_data.get('error', {}).get('message', error_data)}"
                    )
                except:
