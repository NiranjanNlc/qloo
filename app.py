...
st.set_page_config(
    page_title="Qloo Supermarket Optimizer", page_icon="🛒", layout="wide"
)
...
def create_kpi_card(
    title, current, target, previous, unit, status, trend, change_percent=None
):
                ...
                gauge={
                    ...
                    "bar": {
                        "color": (
                            "#198754"
                            if status == "good"
                            else "#fd7e14" if status == "warning" else "#dc3545"
                        )
                    },
                    "steps": [
...
if metrics_data:
    ...
    with col1:
        st.markdown(
            f"**Last Updated:** {datetime.fromisoformat(metrics_data['timestamp']).strftime('%Y-%m-%d %H:%M:%S')}"
        )
    with col2:
    ...
    with col1:
        ...
        st.metric(
            "Requests/min",
            requests_per_min,
            delta=f"+{requests_per_min-20}" if requests_per_min > 20 else None,
        )
    ...
    with col2:
        ...
        st.metric(
            "CPU Usage",
            f"{cpu_usage:.1f}%",
            delta=f"+{cpu_usage-10:.1f}%" if cpu_usage > 10 else None,
        )
    ...
    with col3:
        ...
        st.metric(
            "DB Query Time",
            f"{db_query_time}ms",
            delta=f"-{30-db_query_time}ms" if db_query_time < 30 else None,
        )
    ...
    st.error(
        "🚨 Unable to connect to metrics API. Please ensure the API service is running."
    )
