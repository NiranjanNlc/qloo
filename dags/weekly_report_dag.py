...
def extract_transaction_data(**context):
    ...
    logging.info(f"Extracting transaction data for week: {week_start.date()} to {week_end.date()}")
    ...
    try:
        ...
        # Save extracted data
        output_file = data_dir / f"transactions_week_{week_start.strftime('%Y%m%d')}.json"
...
def generate_association_rules(**context):
    ...
    extraction_metadata = context['task_instance'].xcom_pull(
        task_ids='extract_transaction_data',
        key='extraction_metadata'
    )
    
    logging.info(f"Generating association rules for {extraction_metadata['transaction_count']} transactions")
    ...
    try:
        ...
        rules_file = Path('/opt/airflow/dags/data') / f"association_rules_week_{week_start.strftime('%Y%m%d')}.json"
        ...
        context['task_instance'].xcom_push(
            ...
            value={
                ...
                'high_confidence_rules': len([r for r in serializable_rules if r['confidence'] > 0.8])
            }
        )
...
def generate_combo_offers(**context):
    ...
    rules_metadata = context['task_instance'].xcom_pull(
        task_ids='generate_association_rules',
        key='rules_metadata'
    )
    ...
    try:
        ...
        mock_products = [
            type('Product', (), {'id': i, 'name': f'Product {i}', 'category': f'Category {i//10}'})()
            for i in range(1, 60)
        ]
        ...
        # Save combos
        week_start = datetime.fromisoformat(extraction_metadata['week_start'])
        combos_file = Path('/opt/airflow/dags/data') / f"combos_week_{week_start.strftime('%Y%m%d')}.json"
        ...
        context['task_instance'].xcom_push(
            ...
            value={
                ...
                'high_confidence_combos': len([c for c in serializable_combos if c['confidence_score'] > 0.8])
            }
        )
...
def generate_weekly_report(**context):
    ...
    combos_metadata = context['task_instance'].xcom_pull(
        task_ids='generate_combo_offers',
        key='combos_metadata'
    )
    ...
    try:
        ...
        data = [[combo.confidence_score for combo in combos[:10]],
    ...
    try:
        if is_success:
            ...
            report_metadata = context['task_instance'].xcom_pull(
                task_ids='generate_weekly_report',
                key='report_metadata'
            )
                ...
                "attachments": [
                    {
                        ...
                        "fields": [
                            {
                                "title": "Week",
                                "value": datetime.fromisoformat(report_metadata['week_start']).strftime('%Y-%m-%d'),
                                "short": True
                            },
                            {
...
start_task >> extract_data_task >> generate_rules_task >> generate_combos_task >> generate_report_task

# Notification tasks
generate_report_task >> notify_success_task >> end_task
[extract_data_task, generate_rules_task, generate_combos_task, generate_report_task] >> notify_failure_task 