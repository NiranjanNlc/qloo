    ...
    for pattern_name, pattern_info in patterns.items():
        if random.random() < pattern_info["probability"]:
            ...
            num_items = random.randint(
                pattern_info["min_items"], pattern_info["max_items"]
            )
            selected_products = random.sample(
                pattern_info["products"], min(num_items, len(pattern_info["products"]))
            )
...
def generate_transactions(
    num_transactions: int = 1000, start_date: datetime = None, end_date: datetime = None
) -> pd.DataFrame:
    """
    Generate sample transaction data.
    ...
    for transaction_id in range(1, num_transactions + 1):
        ...
        if pattern_mult > 1.0:
            ...
            extra_products = random.sample(
                all_products, min(additional_items, len(all_products))
            )
...
def save_transaction_data(
    transactions_df: pd.DataFrame, filename: str = "data/sample_transactions.csv"
):
    ...
    print(
        f"  - Date range: {transactions_df['timestamp'].min()} to {transactions_df['timestamp'].max()}"
    )
