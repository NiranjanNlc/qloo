...
def validate_catalog_data(df: pd.DataFrame) -> Tuple[bool, list]:
    ...
    if not long_names.empty:
        errors.append(f"Found {len(long_names)} product names longer than 100 characters")
...
def clean_catalog_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_cleaned['product_id'] = pd.to_numeric(df_cleaned['product_id'], errors='coerce').astype('Int64')
...
def load_catalog_to_database(csv_path: str, db_path: str, replace_existing: bool = True) -> int:
    """
    ...
    try:
        ...
        for product in sample_products:
            logger.info(f"  ID: {product[0]}, Name: {product[1]}, Category: {product[2]}")
...
def get_catalog_statistics(db_path: str) -> dict:
    ...
    try:
        ...
        logger.info(f"Catalog statistics: {total_products} products across {len(categories)} categories")
        return stats
...
""")
    ...
    try:
        ...
        print(f"📊 Total products in database: {stats.get('total_products', 'Unknown')}")
