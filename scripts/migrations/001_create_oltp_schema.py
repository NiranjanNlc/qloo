...
class Migration001:
    ...
    def record_migration(
        self, conn, execution_time, status="success", error_message=None
    ):
    ...
    def validate_schema(self, conn):
        ...
        cursor.execute(
            "SELECT EXISTS(SELECT 1 FROM information_schema.schemata WHERE schema_name = 'pos_oltp')"
        )
    ...
    def up(self):
        ...
        try:
            ...
            conn.close()
            logger.info(
                f"Migration {self.migration_name} completed successfully in {execution_time:.2f} seconds"
            )
...
def main():
    ...
    if args.dry_run:
        print(
            f"Would execute migration {migration.migration_name} with action: {args.action}"
        )
        if args.action == "down":
            print("Rollback SQL:")
