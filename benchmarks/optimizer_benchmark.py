...
class OptimizerBenchmark:
    ...
    def generate_mock_data(self, num_skus: int, num_transactions: int = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        logger.info(f"Generating mock data: {num_skus} SKUs, {num_transactions} transactions")
        
        # Generate product catalog
        np.random.seed(42)  # For reproducible results
        
        categories = ['Dairy', 'Produce', 'Meat', 'Beverages', 'Snacks', 'Bakery', 
                     'Frozen', 'Canned', 'Personal Care', 'Household']
        ...
        logger.info(f"Generated {len(catalog_df)} products and {len(transactions_df)} transaction records")
        return catalog_df, transactions_df
    
    @memory_profiler.profile
    def benchmark_optimizer_initialization(self, catalog_df: pd.DataFrame, transactions_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        logger.info(f"Initialization completed in {result['initialization_time']:.2f}s, memory: {result['memory_usage_mb']:.1f}MB")
        return result
    
    def benchmark_association_engine(self, transactions_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        logger.info(f"Association engine: training {training_time:.2f}s, {len(rules)} rules generated")
        return result
    
    def benchmark_layout_optimization(self, optimizer: SupermarketLayoutOptimizer) -> Dict[str, Any]:
        ...
        logger.info(f"Layout optimization: {optimization_time:.2f}s, {len(recommendations)} recommendations")
        return result
    
    def benchmark_heuristic_scoring(self, optimizer: SupermarketLayoutOptimizer) -> Dict[str, Any]:
        ...
        logger.info(f"Heuristic scoring: {scoring_time:.2f}s, {len(scores)} scores calculated")
        return result
    ...
    def run_full_benchmark_suite(self) -> Dict[str, Any]:
        ...
        for num_skus in sku_counts:
            ...
            try:
                ...
                init_results = self.benchmark_optimizer_initialization(catalog_df, transactions_df)
                ...
                benchmark_results[sku_key] = {
                    ...
                    'meets_target': overall_time <= self.targets.get(sku_key, float('inf')),
                ...
                # Log results
                target_met = "✅ MEETS TARGET" if benchmark_results[sku_key]['meets_target'] else "❌ EXCEEDS TARGET"
    ...
    def _convert_to_transaction_matrix(self, transactions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _extract_hotspots(self, stats: pstats.Stats) -> List[Dict[str, Any]]:
        ...
        for func_key, (call_count, total_time, cumulative_time, callers) in stats.stats.items():
            ...
            if any(module in filename for module in ['layout_optimizer', 'association_engine', 'optimization_heuristics']):
    ...
    def _generate_summary_report(self, benchmark_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if '10k_skus' in benchmark_results and 'error' not in benchmark_results['10k_skus']:
            result_10k = benchmark_results['10k_skus']
            
            if result_10k['meets_target']:
                summary['recommendations'].append("✅ 10k SKU target met - performance is acceptable")
            else:
                summary['recommendations'].append("❌ 10k SKU target exceeded - optimization needed")
    ...
    def _create_performance_charts(self, benchmark_results: Dict[str, Any], timestamp: str):
        ...
        try:
            ...
            plt.subplot(2, 2, 1)
            plt.plot(sku_counts, overall_times, 'bo-', label='Actual Time', linewidth=2, markersize=8)
            ...
            plt.subplot(2, 2, 2)
            components = ['initialization', 'association_engine', 'layout_optimization', 'heuristic_scoring']
            colors = ['skyblue', 'lightgreen', 'lightcoral', 'lightyellow']
            
            for i, sku_key in enumerate(['1k_skus', '5k_skus', '10k_skus']):
                if sku_key in benchmark_results and 'error' not in benchmark_results[sku_key]:
                    ...
                    for j, (component, time_val) in enumerate(zip(components, times)):
                        plt.bar(i, time_val, bottom=bottom, color=colors[j], 
            ...
            for sku_key in ['1k_skus', '5k_skus', '10k_skus']:
                if sku_key in benchmark_results and 'error' not in benchmark_results[sku_key]:
                    memory_usage.append(benchmark_results[sku_key]['initialization']['memory_usage_mb'])
                else:
            ...
            plt.bar(range(len(compliance_data)), compliance_data, color=colors_compliance)
            plt.xlabel('SKU Count')
...
def main():
    ...
    if 'target_compliance' in summary:
        ...
        print(f"🎯 Target compliance: {compliance['compliant_runs']}/{compliance['total_runs']} ({rate:.1f}%)")
