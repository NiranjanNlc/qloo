...
class TestPriceAPIStub:
    ...
    def test_suggest_discount_for_combo_basic(self):
        ...
        assert (
            suggestion.min_discount
            <= suggestion.suggested_discount_percent
            <= suggestion.max_discount
        )
    ...
    def test_discount_reason_generation(self):
        ...
        assert (
            "Very high association confidence" in suggestion.reason
            or "high" in suggestion.reason.lower()
        )
        assert (
            "strong" in suggestion.reason.lower() or "good" in suggestion.reason.lower()
        )
    ...
    def test_confidence_calculation(self):
        ...
        low_conf_suggestion = self.price_api.suggest_discount_for_combo(
            combo_id="low_conf_test", product_ids=[1, 2], confidence_score=0.8, lift=1.1
        )

        assert (
            high_conf_suggestion.confidence_score
            >= low_conf_suggestion.confidence_score
        )
