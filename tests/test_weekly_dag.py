...
class TestWeeklyReportDAG:
    ...
    def test_extract_transaction_data_idempotency(self, mock_context, temp_data_dir):
        ...
        with patch('weekly_report_dag.Path') as mock_path:
            ...
            mock_path.side_effect = lambda x: temp_data_dir if '/opt/airflow/dags/data' in str(x) else Path(x)
    ...
    def test_generate_association_rules_with_valid_data(self, mock_context, temp_data_dir):
        """Test association rules generation with valid transaction data."""
        
        # Mock previous task output
        sample_transactions = [
            {"transaction_id": "20240105_1", "date": "2024-01-05", "products": [1, 2, 3], "total_amount": 25.50},
            {"transaction_id": "20240105_2", "date": "2024-01-05", "products": [1, 4, 5], "total_amount": 35.75},
            {"transaction_id": "20240105_3", "date": "2024-01-05", "products": [2, 3, 6], "total_amount": 45.25}
        ]
    ...
    def test_generate_combo_offers_idempotency(self, mock_context, temp_data_dir):
        ...
        sample_rules = [
            {"antecedent": [1], "consequent": [2], "confidence": 0.8, "support": 0.1, "lift": 1.5},
            {"antecedent": [2], "consequent": [3], "confidence": 0.7, "support": 0.15, "lift": 1.2}
        ]
    ...
    def test_generate_weekly_report_with_valid_inputs(self, mock_context, temp_data_dir):
