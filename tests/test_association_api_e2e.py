...
class TestAssociationAPIE2E:
    ...
    def test_combos_endpoint_basic(self, api_client):
        ...
        assert data["total_combos"] >= 20, f"Expected at least 20 combos, got {data['total_combos']}"
        assert len(data["combos"]) >= 20, f"Expected at least 20 combos in list, got {len(data['combos'])}"
    ...
    def test_combos_endpoint_with_filters(self, api_client):
        ...
        response = api_client.get("/combos", params={"min_confidence": 0.7, "limit": 30})
        assert response.status_code == 200
        data = response.json()
        ...
        # Test with sorting
        response = api_client.get("/combos", params={"sort_by": "lift", "sort_desc": True, "limit": 25})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_api_error_handling(self, api_client):
        ...
        response = api_client.get("/combos", params={"min_confidence": 2.0})  # Invalid confidence
    ...
    def _validate_combo_object_schema(self, combo: Dict[str, Any]):
        ...
        required_fields = [
            "combo_id", "name", "products", "product_names",
            "confidence_score", "support", "lift", "is_active"
        ]
        ...
        if "expected_discount_percent" in combo and combo["expected_discount_percent"] is not None:
    ...
    def _validate_associations_response_schema(self, data: Dict[str, Any]):
        ...
        required_fields = ["product_id", "product_name", "associations", "total_associations"]
