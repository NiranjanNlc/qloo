...
# Page configuration
st.set_page_config(
    page_title="Shelf Placements Analysis", page_icon="📋", layout="wide"
)

st.title("📋 Shelf Placements Analysis")
st.markdown(
    "Interactive analysis of product shelf placements with optimization recommendations"
)
...
def generate_mock_placement_data():
    ...
    for product_id in range(1, 201):  # 200 products
        ...
        shelf_priority = np.random.choice(
            shelf_priorities, p=[0.3, 0.5, 0.2]
        )  # Bias toward eye/reach level
        ...
        confidence_score = np.clip(
            base_score + shelf_boost + traffic_boost + np.random.normal(0, 0.05), 0, 1
        )
        ...
        placement_data.append(
            {
                ...
                "shelf_level": {1: "Eye Level", 2: "Reach Level", 3: "Stoop Level"}[
                    shelf_priority
                ],
...
def create_interactive_datatable(df, filters):
    ...
    if filters["shelf_levels"]:
        filtered_df = filtered_df[
            filtered_df["shelf_level"].isin(filters["shelf_levels"])
        ]
...
def create_confidence_distribution_chart(df):
    ...
    fig.update_layout(showlegend=True, height=400, bargap=0.1)
...
def create_placement_scatter(df):
    ...
    fig.update_layout(
        xaxis_title="X Coordinate", yaxis_title="Y Coordinate", height=500
    )
...
def create_optimization_recommendations(df):
    ...
    high_perf_low_shelf = df[
        (df["confidence_score"] > 0.8) & (df["shelf_priority"] == 3)
    ]
...
def main():
    ...
    filters = {
        ...
        "zones": st.sidebar.multiselect(
            "Zones", options=df["zone"].unique(), default=[]
        ),
        "shelf_levels": st.sidebar.multiselect(
            "Shelf Levels", options=df["shelf_level"].unique(), default=[]
        ),
    ...
    with col4:
        ...
        st.metric(
            "Avg Association Strength", f"{summary['avg_association_strength']:.3f}"
        )
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["Distribution", "Placement Map", "Zone Analysis", "Shelf Analysis"]
    )

    with tab1:
        st.plotly_chart(
            create_confidence_distribution_chart(filtered_df), use_container_width=True
        )
    ...
    with tab2:
        ...
        st.info(
            "💡 Bubble size represents weekly sales, color represents confidence score"
        )
    ...
    if recommendations:
        for i, rec in enumerate(recommendations):
            ...
            with st.expander(
                f"{priority_color[rec['priority']]} {rec['type']} ({rec['count']} items)"
            ):
        ...
        st.success(
            "🎉 No optimization issues found! Current placement strategy appears optimal."
        )
