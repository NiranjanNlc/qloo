...
def create_store_layout_visualization(optimizer, recommendations=None):
    ...
    for zone in optimizer.store_zones.values():
        fig.add_shape(
            type="rect",
            x0=zone.x_range[0], y0=zone.y_range[0],
            x1=zone.x_range[1], y1=zone.y_range[1],
    ...
    for product_id, location in optimizer.current_layout.items():
        ...
        fig.add_trace(go.Scatter(
            ...
            showlegend=True if product_id == min(optimizer.current_layout.keys()) else False,
...
def display_recommendations_table(recommendations, catalog_df):
    ...
    if not recommendations:
        st.info("No recommendations generated. Try adjusting the optimization parameters.")
        return
    ...
    for rec in recommendations:
        rec_data.append({
            'Product': rec.current_location.product_name if rec.current_location else 'Unknown',
            'Category': rec.current_location.category if rec.current_location else 'Unknown',
            'Current Zone': rec.current_location.zone if rec.current_location else 'Unknown',
...
def display_layout_metrics(optimizer):
    ...
    if report['zone_utilization']:
        ...
        zone_df = pd.DataFrame(
            list(report['zone_utilization'].items()),
            columns=['Zone', 'Products']
        )
...
def display_qloo_integration(optimizer, selected_product_id):
    ...
    if not optimizer.qloo_client:
        st.warning("Qloo API not available. Layout optimization is based only on transaction data.")
        return
    
    st.subheader("🔮 Qloo AI Recommendations")
    
    try:
        qloo_recommendations = optimizer.get_qloo_recommendations(selected_product_id, limit=5)
...
def main():
    ...
    show_recommendations = st.sidebar.checkbox("Show Layout Recommendations", value=True)
    ...
    with col1:
        ...
        layout_fig = create_store_layout_visualization(optimizer, recommendations if show_recommendations else None)
    ...
    with col2:
        ...
        if report['association_score'] < 0.5:
            st.warning("⚠️ Layout efficiency is low. Consider implementing recommendations.")
    ...
    if show_recommendations and recommendations:
        ...
        medium_priority = sum(1 for rec in recommendations if 0.4 <= rec.confidence_score < 0.7)
            ...
            'recommendations': [
                {
                    ...
                    'product_name': rec.current_location.product_name if rec.current_location else 'Unknown',
                    'current_zone': rec.current_location.zone if rec.current_location else 'Unknown',
