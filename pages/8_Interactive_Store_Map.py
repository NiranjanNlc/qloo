...
# Page configuration
st.set_page_config(page_title="Interactive Store Map", page_icon="🗺️", layout="wide")
...
def generate_traffic_flow_data():
    ...
    for x in x_coords:
        for y in y_coords:
            ...
            traffic_data.append({"x": x, "y": y, "traffic_count": traffic})
...
def create_store_map(sections, placements_df, traffic_df, view_mode="layout"):
    ...
    if view_mode in ["layout", "performance"]:
        colors = (
            placements_df["performance_score"]
            if view_mode == "performance"
            else ["blue"] * len(placements_df)
        )
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="X Coordinate (feet)", range=[-1, 14], constrain="domain"),
        yaxis=dict(
            title="Y Coordinate (feet)", range=[-1, 17], scaleanchor="x", scaleratio=1
        ),
        showlegend=True,
        height=600,
...
def calculate_optimization_metrics(sections, placements_df, traffic_df):
    ...
    # Traffic efficiency
    high_traffic_areas = traffic_df[
        traffic_df["traffic_count"] > traffic_df["traffic_count"].quantile(0.8)
    ]
...
def main():
    ...
    st.markdown(
        "Visualize store layout, analyze traffic patterns, and optimize product placements"
    )
    ...
    if show_analytics:
        with col2:
            ...
            # Calculate metrics
            metrics, section_performance = calculate_optimization_metrics(
                sections, placements_df, traffic_df
            )
            ...
            st.metric(
                "Avg Performance",
                f"{metrics['avg_performance']:.3f}",
                delta=f"{(metrics['avg_performance'] - 0.8):.3f}",
            )
            st.metric(
                "Weekly Sales", f"${metrics['total_weekly_sales']:,}", delta="12.5%"
            )
            st.metric(
                "Profit Margin", f"{metrics['avg_profit_margin']:.1%}", delta="2.3%"
            )
            st.markdown("</div>", unsafe_allow_html=True)
            ...
            top_products = placements_df.nlargest(3, "performance_score")[
                ["product_name", "performance_score", "section"]
            ]
            for _, product in top_products.iterrows():
                st.markdown(
                    f"• **{product['product_name']}** ({product['section']}) - {product['performance_score']:.3f}"
                )
    ...
    if show_suggestions:
        ...
        suggestions = generate_optimization_suggestions(
            placements_df, traffic_df, sections
        )

        for suggestion in suggestions:
            priority_color = {"High": "#ff4444", "Medium": "#ffaa00", "Low": "#44ff44"}[
                suggestion["priority"]
            ]
    ...
    with col2:
        # Sales by section
        section_sales = (
            placements_df.groupby("section")["weekly_sales"].sum().reset_index()
        )
        fig_sales = px.pie(
            section_sales,
            values="weekly_sales",
