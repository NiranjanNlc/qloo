...
try:
    ...
    st.error("streamlit-autorefresh not installed. Run: pip install streamlit-autorefresh")
    st.stop()
...
# Page configuration
st.set_page_config(
    page_title="DAG Status Monitor",
    page_icon="📊",
    layout="wide"
)

st.title("📊 DAG Status Monitor")
st.markdown("Real-time monitoring of Airflow DAG executions and weekly report generation")
...
class DAGStatusMonitor:
    ...
    def _generate_mock_dag_status(self) -> Dict[str, Any]:
        ...
        next_run = now.replace(hour=2, minute=0, second=0, microsecond=0) + timedelta(days=days_until_friday)
    ...
    def _generate_mock_task_status(self, dag_run_id: str) -> List[Dict[str, Any]]:
        ...
        for i, task_id in enumerate(tasks):
            ...
            start_time = current_time - timedelta(minutes=len(tasks) - i) if state != 'queued' else None
            end_time = current_time - timedelta(minutes=len(tasks) - i - 1) if state == 'success' else None
...
def render_dag_overview(dag_status: Dict[str, Any]):
    ...
    with col4:
        ...
        st.metric(
            "Next Run",
            f"{days}d {hours}h",
            delta=next_run.strftime("%Y-%m-%d %H:%M")
        )
...
def render_current_run_status(task_status: List[Dict[str, Any]]):
    ...
    for i, task in enumerate(task_status):
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color=color,
            name=task['state'],
            text=f"{task['state']} ({width:.1f}m)" if task['duration'] else task['state'],
            textposition='inside',
    ...
    display_df = task_df[['task_id', 'state', 'start_date', 'end_date', 'duration']].copy()
    display_df['start_date'] = display_df['start_date'].dt.strftime('%H:%M:%S') if display_df['start_date'].notna().any() else None
    display_df['end_date'] = display_df['end_date'].dt.strftime('%H:%M:%S') if display_df['end_date'].notna().any() else None
    display_df['duration'] = display_df['duration'].apply(lambda x: f"{x:.1f}s" if pd.notna(x) else "-")
...
def render_recent_runs(dag_runs: List[Dict[str, Any]]):
    ...
    display_runs['execution_date'] = display_runs['execution_date'].dt.strftime('%Y-%m-%d %H:%M')
...
def render_performance_metrics(dag_runs: List[Dict[str, Any]]):
    ...
    # Calculate metrics
    successful_runs = [run for run in dag_runs if run['state'] == 'success' and run['duration']]
