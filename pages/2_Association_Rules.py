...
def load_product_catalog():
    ...
    try:
        ...
        st.error(
            "Product catalog not found. Please ensure data/grocery_catalog.csv exists."
        )
        return pd.DataFrame()
...
def load_transaction_data():
    ...
    try:
        ...
        try:
            ...
            st.error(
                "Could not generate transaction data. Please run: python scripts/generate_sample_transactions.py"
            )
            return pd.DataFrame()
...
def display_association_rules(engine, catalog_df):
    ...
    if not catalog_df.empty:
        ...
        for _, row in catalog_df.iterrows():
            product_options[f"{row['product_name']} (ID: {row['product_id']})"] = row[
                "product_id"
            ]

        selected_product_display = st.selectbox(
            "Choose a product:", options=list(product_options.keys()), index=0
        )
        ...
        if associations:
            st.subheader(
                f"📊 Products Associated with {format_product_name(selected_product_id, catalog_df)}"
            )
            ...
            for assoc_product_id, lift_score in associations:
                association_data.append(
                    {
                        "Associated Product": format_product_name(
                            assoc_product_id, catalog_df
                        ),
            ...
            styled_df = assoc_df.style.applymap(
                color_strength, subset=["Strength Category"]
            )
            ...
            with st.expander("🔍 Detailed Association Rules"):
                ...
                if rule_details:
                    for i, rule in enumerate(rule_details[:10]):  # Show top 10 rules
                        antecedent_names = [
                            format_product_name(pid, catalog_df)
                        ...
                        st.write(
                            f"**Rule {i+1}:** {', '.join(antecedent_names)} → {', '.join(consequent_names)}"
                        )
            ...
            st.info(
                f"No strong associations found for {format_product_name(selected_product_id, catalog_df)}. Try adjusting the parameters below."
            )
...
def main():
    ...
    if transactions_df.empty:
        st.error(
            "No transaction data available. Please generate transaction data first."
        )
