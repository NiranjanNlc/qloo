...
# Page configuration
st.set_page_config(page_title="Combo Offers", page_icon="🎁", layout="wide")
...
st.markdown(
    "Create, manage, and export product combination offers with AI-powered recommendations"
)
...
def generate_mock_combo_data():
    ...
    for i, template in enumerate(combo_templates):
        ...
        theme_boost = {
            ...
            "health": 0.07,
            "family": 0.02,
        }.get(template["theme"], 0)
        confidence = min(
            0.98, base_confidence + theme_boost + np.random.normal(0, 0.02)
        )
...
def render_combo_card(combo_data, all_products):
    ...
    # Category tags
    category_tags = "".join(
        [f'<span class="category-tag">{cat}</span>' for cat in combo.category_mix]
    )
    ...
    for combo_data in combo_list:
        ...
        if format_type == "detailed":
            combo_export = {
                ...
                "metadata": {"last_performance": combo_data["last_performance"]},
            }
        else:  # simplified
            combo_export = {
                "combo_id": combo.combo_id,
...
def main():
    ...
    selected_themes = st.sidebar.multiselect("Themes", options=themes, default=themes)

    # Confidence filter
    confidence_range = st.sidebar.slider(
        "Confidence Range", min_value=0.0, max_value=1.0, value=(0.0, 1.0), step=0.1
    )

    # Status filter
    status_filter = st.sidebar.selectbox(
        "Status", options=["All", "Active Only", "Inactive Only"]
    )

    # Discount range filter
    discount_range = st.sidebar.slider(
        "Discount Range (%)", min_value=0.0, max_value=30.0, value=(0.0, 30.0), step=1.0
    )
    ...
    for combo_data in combo_list:
        ...
        # Discount filter
        if not (
            discount_range[0] <= combo.expected_discount_percent <= discount_range[1]
        ):
            continue
    ...
    avg_confidence = (
        np.mean([c["combo"].confidence_score for c in filtered_combos])
        if filtered_combos
        else 0
    )
    ...
    with col3:
        if filtered_combos:
            avg_discount = np.mean(
                [c["combo"].expected_discount_percent for c in filtered_combos]
            )
    ...
    with col4:
        ...
        category_count = len(
            set([cat for c in filtered_combos for cat in c["combo"].category_mix])
        )
    ...
    if filtered_combos:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["Performance", "Distribution", "Themes", "Revenue"]
        )
        ...
        with tab1:
            ...
            top_combos = df.nlargest(5, "popularity")[
                ["name", "confidence", "lift", "revenue", "theme"]
            ]
        ...
        with tab2:
            ...
            with col1:
                ...
                fig = px.histogram(
                    df, x="confidence", title="Confidence Score Distribution", nbins=15
                )
                st.plotly_chart(fig, use_container_width=True)
            ...
            with col2:
                ...
                fig = px.histogram(
                    df, x="discount", title="Discount Distribution", nbins=15
                )
                st.plotly_chart(fig, use_container_width=True)
        ...
        with tab4:
            # Revenue analysis
            fig = px.box(
                df, x="theme", y="revenue", title="Revenue Distribution by Theme"
            )
            st.plotly_chart(fig, use_container_width=True)
    ...
    sorted_combos = sorted(
        filtered_combos, key=sort_key_map[sort_by], reverse=sort_desc
    )
    ...
    if sorted_combos:
        ...
        if total_pages > 1:
            st.markdown(
                f"*Showing page {page + 1} of {total_pages} ({start_idx + 1}-{end_idx} of {len(sorted_combos)} combos)*"
            )

    else:
        st.info(
            "🔍 No combos match the current filters. Try adjusting your filter criteria."
        )
