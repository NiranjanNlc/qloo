...
# Page configuration
st.set_page_config(
    page_title="Product Flow Analysis",
    page_icon="🌊",
    layout="wide"
)

st.title("🌊 Product Flow Analysis")
st.markdown("Interactive Sankey diagrams showing customer flow patterns and product associations")
...
def generate_mock_flow_data():
    ...
    sections = {
        ...
        'produce_fresh': {'name': 'Fresh Produce', 'traffic_weight': 0.85, 'category': 'fresh'},
        'produce_packaged': {'name': 'Packaged Produce', 'traffic_weight': 0.65, 'category': 'fresh'},
        'dairy_milk_eggs': {'name': 'Milk & Eggs', 'traffic_weight': 0.90, 'category': 'dairy'},
        'dairy_cheese_yogurt': {'name': 'Cheese & Yogurt', 'traffic_weight': 0.70, 'category': 'dairy'},
        'meat_fresh': {'name': 'Fresh Meat', 'traffic_weight': 0.75, 'category': 'meat'},
        'meat_deli': {'name': 'Deli Counter', 'traffic_weight': 0.60, 'category': 'meat'},
        'center_aisle_1': {'name': 'Beverages & Snacks', 'traffic_weight': 0.80, 'category': 'packaged'},
        'center_aisle_2': {'name': 'Pantry Staples', 'traffic_weight': 0.70, 'category': 'packaged'},
        'center_aisle_3': {'name': 'Household Items', 'traffic_weight': 0.50, 'category': 'household'},
        'bakery': {'name': 'Bakery', 'traffic_weight': 0.55, 'category': 'fresh'},
        'frozen': {'name': 'Frozen Foods', 'traffic_weight': 0.65, 'category': 'frozen'},
    ...
    for customer_id in range(n_customers):
        ...
        for _ in range(max_sections):
            ...
            possible_next = [
                (target, weight) for source, target, weight in flow_patterns 
...
def create_customer_flow_sankey(sections, flow_patterns, min_flow_strength=0.1):
    ...
    node_colors = [category_colors.get(sections[section]['category'], '#CCCCCC') 
    ...
    fig = go.Figure(data=[go.Sankey(
        node=dict(
            ...
            label=section_names,
            color=node_colors,
            x=[0.1 if 'entrance' in sections[s]['name'].lower() else 
               0.9 if 'checkout' in sections[s]['name'].lower() else 
               0.3 + (i % 3) * 0.2 for i, s in enumerate(all_sections)],
...
def create_product_association_sankey(product_categories, product_flows, min_strength=0.3):
    ...
    fig.update_layout(
        title_text="Product Association Flows",
        title_x=0.5,
        font_size=10,
        height=700
    )
...
def create_category_to_section_flow(sections, product_categories):
    ...
    # Create colors
    category_colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A', '#98D8C8', '#F7DC6F', '#BB8FCE', '#85C1E9', '#DDA0DD']
    ...
    fig = go.Figure(data=[go.Sankey(
        node=dict(
            ...
            label=all_nodes,
            color=node_colors,
            x=[0.1 if i < len(categories) else 0.9 for i in range(len(all_nodes))],
...
def analyze_flow_metrics(customer_journeys, sections):
    ...
    efficiency_scores = [u/t if t > 0 else 0 for u, t in zip(unique_sections, total_sections)]
...
def main():
    ...
    if flow_type == "Customer Section Flow":
        ...
        st.markdown("This Sankey diagram shows how customers move between different store sections")
        ...
        st.markdown("Sankey diagram showing which products are frequently bought together")
        ...
        if selected_categories:
            ...
            filtered_flows = [
                (p1, p2, s) for p1, p2, s in data['product_flows']
                if p1 in filtered_products and p2 in filtered_products
            ]
            
            filtered_categories = {k: v for k, v in data['product_categories'].items() 
        ...
        strong_associations = [
            (p1, p2, s) for p1, p2, s in filtered_flows 
            if s >= min_association_strength
        ]
        ...
        # Create and display Sankey
        fig = create_category_to_section_flow(data['sections'], data['product_categories'])
        st.plotly_chart(fig, use_container_width=True)
        ...
        with col1:
            ...
            st.metric("Median Journey Length", f"{metrics['median_journey_length']:.1f}")
        
        with col2:
            st.metric("Most Visited Section", data['sections'][metrics['most_visited_section']]['name'])
            st.metric("Least Visited Section", data['sections'][metrics['least_visited_section']]['name'])
        ...
        for i, journey in enumerate(sample_journeys):
            with st.expander(f"Customer {journey['customer_id']} Journey"):
                journey_sections = [data['sections'][s]['name'] for s in journey['journey']]
