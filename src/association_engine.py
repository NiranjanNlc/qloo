...
class AprioriAssociationEngine(AssociationEngine):
    ...
    def __init__(self, min_support: float = 0.1, min_confidence: float = 0.5, 
                 min_lift: float = 1.0, max_itemset_size: int = 3):
        """
    ...
    def train(self, transactions_df: pd.DataFrame):
        ...
        print(f"Training complete! Found {len(self.association_rules)} association rules.")
    ...
    def _generate_candidates(self, frequent_itemsets: Dict[frozenset, int]) -> List[frozenset]:
    ...
    def _all_subsets_frequent(self, candidate: frozenset, frequent_itemsets: Dict[frozenset, int]) -> bool:
                                ...
                                if consequent_support > 0:
                                    ...
                                    if lift >= self.min_lift:
                                        rules.append({
                                            ...
                                            'lift': lift,
                                            'conviction': self._calculate_conviction(confidence, expected_confidence)
                                        })
    ...
    def _calculate_conviction(self, confidence: float, expected_confidence: float) -> float:
    ...
    def get_associations(self, product_id: int, top_k: int = 5) -> List[Tuple[int, float]]:
        """
        ...
        sorted_associations = sorted(unique_associations.items(), key=lambda x: x[1], reverse=True)
