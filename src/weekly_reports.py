...
class WeeklyKPI:
    ...
    def __post_init__(self):
        ...
        if self.previous_value is not None and self.previous_value != 0:
            self.change_percent = ((self.current_value - self.previous_value) / 
                                 abs(self.previous_value)) * 100
            
            # Determine if change is improvement (depends on metric type)
            improvement_metrics = [
                'combos_generated', 'high_confidence_rules', 'avg_confidence',
                'avg_lift', 'total_revenue_potential', 'discount_efficiency'
            ]
            decline_metrics = ['avg_discount_percent', 'processing_time']
            
            if any(metric in self.metric_name.lower() for metric in improvement_metrics):
...
class WeeklyKPICalculator:
    ...
    def calculate_weekly_kpis(self, 
                            ...
                            week_start: Optional[datetime] = None) -> WeeklyKPITable:
        """
    ...
    def _calculate_combo_kpis(self, combos: List[Combo]) -> Dict[str, float]:
        ...
        avg_discount = 0
        if discount_combos:
            avg_discount = sum(c.expected_discount_percent for c in discount_combos) / len(discount_combos)
        
        # Revenue potential (simplified calculation)
        revenue_potential = total_combos * avg_confidence * 500  # $500 per combo estimated
    ...
    def _calculate_summary_stats(self, combos: List[Combo], kpis: List[WeeklyKPI]) -> Dict[str, Any]:
        ...
        return {
            ...
            'top_performing_combos': len([c for c in combos if c.confidence_score >= 0.95]),
...
class WeeklyReport:
    ...
    @classmethod
    def from_yaml_config(cls, config_path: str, combos: List[Combo], 
                        previous_combos: Optional[List[Combo]] = None) -> 'WeeklyReport':
        """
    ...
    def generate_report(self, format_type: str = 'html', output_dir: str = 'reports') -> str:
        """
...
def generate_weekly_report_cli(week: str, config_path: str = None, output_format: str = 'html'):
    """
    ...
    try:
        ...
        print(f"Generating weekly report for week starting: {week_start.strftime('%Y-%m-%d')}")
        ...
        print(f"Loaded {len(mock_combos)} current combos and {len(previous_combos)} previous combos")
        ...
        print("\nReport Summary:")
        print(f"  Week: {kpi_table.week_start_date.strftime('%Y-%m-%d')} to {kpi_table.week_end_date.strftime('%Y-%m-%d')}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--format', default='html', choices=['html', 'markdown', 'json', 'yaml'],
