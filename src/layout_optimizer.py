...
class SupermarketLayoutOptimizer:
    ...
    def load_data(
        ...
        if transactions_df is not None:
            self.association_engine = AprioriAssociationEngine(
                min_support=0.03, min_confidence=0.2, min_lift=1.1, max_itemset_size=3
            )
    ...
    def optimize_layout(
        ...
        return sorted(
            unique_recommendations, key=lambda x: x.confidence_score, reverse=True
        )
    ...
    def _optimize_for_flow(self) -> List[LayoutRecommendation]:
        ...
        if self.association_engine and self.association_engine.is_trained:
            ...
            for (
                itemset_size,
                itemsets,
            ) in self.association_engine.frequent_itemsets.items():
                ...
                if current_zone.foot_traffic_score < 0.8 and popularity > 0.3:
                    ...
                    if best_zone:
                        new_location = ProductLocation(
                            ...
                            x_coordinate=(best_zone.x_range[0] + best_zone.x_range[1])
                            / 2,
                            y_coordinate=(best_zone.y_range[0] + best_zone.y_range[1])
                            / 2,
    ...
    def get_qloo_recommendations(self, product_id: int, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            return self.qloo_client.get_product_recommendations(
                product_name, limit=limit
            )
    ...
    def _find_optimal_location_near(
        ...
        new_x = max(
            0, min(1, target_location.x_coordinate + np.random.uniform(-0.2, 0.2))
        )
        new_y = max(
            0, min(1, target_location.y_coordinate + np.random.uniform(-0.2, 0.2))
        )
        ...
        for zone in self.store_zones.values():
            if (
                zone.x_range[0] <= new_x <= zone.x_range[1]
                and zone.y_range[0] <= new_y <= zone.y_range[1]
            ):
                best_zone = zone
                break
    ...
    def _find_best_zone_for_category(
        self, category: str, min_traffic: float = 0.0
    ) -> Optional[StoreZone]:
        ...
        for zone in self.store_zones.values():
            if (
                zone.foot_traffic_score >= min_traffic
                and zone.foot_traffic_score > best_traffic
            ):
                best_zone = zone
    ...
    def _deduplicate_recommendations(
        self, recommendations: List[LayoutRecommendation]
    ) -> List[LayoutRecommendation]:
    ...
    def generate_layout_report(self) -> Dict:
        ...
        report = {
            ...
            "zones_utilized": len(
                set(loc.zone for loc in self.current_layout.values())
            ),
            ...
            for product_id, location in self.current_layout.items():
                ...
                for neighbor_id in neighbors:
                    ...
                    if neighbor_location:
                        ...
                        edge_weight = self.association_graph[product_id][neighbor_id][
                            "weight"
                        ]
...
class SectionLevelOptimizer:
    ...
    def optimize_store_sections(
        self,
        target_sections: Optional[List[str]] = None,
        optimization_criteria: Optional[Dict] = None,
    ) -> List[SectionOptimizationResult]:
        """
        ...
        for section_id in target_sections:
            ...
            optimization_result = self._optimize_single_section(
                section, optimization_criteria
            )
    ...
    def _optimize_single_section(
        ...
        utilization_improvement = (
            len(optimal_products) / section.capacity - section.current_utilization
        )
    ...
    def _get_products_in_section(self, section: StoreSection) -> List[int]:
        ...
        for product_id, location in self.parent_optimizer.current_layout.items():
            ...
            if (
                location.zone == section.parent_zone
                and section.x_range[0] <= location.x_coordinate <= section.x_range[1]
                and section.y_range[0] <= location.y_coordinate <= section.y_range[1]
            ):
    ...
    def _get_candidate_products(
        ...
        for _, product in self.parent_optimizer.product_catalog.iterrows():
            ...
            if (
                category in section.optimal_categories
                or len(section.optimal_categories) == 0
            ):
            ...
            if self.parent_optimizer.association_graph:
                neighbors = list(
                    self.parent_optimizer.association_graph.neighbors(product_id)
                )
                for neighbor_id in neighbors:
                    neighbor_location = self.parent_optimizer.current_layout.get(
                        neighbor_id
                    )
                    if (
                        neighbor_location
                        and neighbor_location.category in section.optimal_categories
                    ):
                        edge_data = self.parent_optimizer.association_graph[product_id][
                            neighbor_id
                        ]
    ...
    def _score_products_for_section(
        self, candidates: List[int], section: StoreSection, criteria: Dict
    ) -> Dict[int, float]:
        ...
        for product_id in candidates:
            ...
            if self.parent_optimizer.product_catalog is not None:
                ...
                if not product_row.empty:
                    ...
                    if (
                        criteria.get("prioritize_high_margin")
                        and section.section_type == "endcap"
                    ):
                        score += 5.0

            # Association score - products with strong associations to section products
            if (
                criteria.get("maximize_cross_selling")
                and self.parent_optimizer.association_graph
            ):
                ...
                for section_product in section_products:
                    if self.parent_optimizer.association_graph.has_edge(
                        product_id, section_product
                    ):
                        edge_data = self.parent_optimizer.association_graph[product_id][
                            section_product
                        ]
            ...
            if (
                self.parent_optimizer.association_engine
                and self.parent_optimizer.association_engine.is_trained
            ):
                ...
                for (
                    itemset_size,
                    itemsets,
                ) in self.parent_optimizer.association_engine.frequent_itemsets.items():
                    if itemset_size == 1:
                        for itemset, count in itemsets.items():
                            if list(itemset)[0] == product_id:
                                popularity = count / len(
                                    self.parent_optimizer.association_engine.transactions
                                )
    ...
    def _select_optimal_product_mix(
        self,
    ...
    def _calculate_performance_lift(
        self,
        current_products: List[int],
            ...
            for i, product1 in enumerate(current_products):
                for product2 in current_products[i + 1 :]:
                    if self.parent_optimizer.association_graph.has_edge(
                        ...
                        edge_data = self.parent_optimizer.association_graph[product1][
                            product2
                        ]
        ...
        if len(current_products) > 1:
            current_strength /= (
                len(current_products) * (len(current_products) - 1)
            ) / 2
        if len(optimal_products) > 1:
            optimal_strength /= (
                len(optimal_products) * (len(optimal_products) - 1)
            ) / 2
        ...
        traffic_factor = (
            self.parent_optimizer.store_zones[section.parent_zone].foot_traffic_score
            * section.foot_traffic_multiplier
        )

        return round(lift * traffic_factor, 3)

    def _generate_section_reasoning(
        self,
        section: StoreSection,
        products_to_add: List[int],
        products_to_remove: List[int],
    ...
    def get_section_performance_report(self) -> Dict[str, Any]:
        ...
        for section in self.store_sections.values():
            ...
            elif section.section_type == "endcap" and section.current_utilization < 0.9:
                report["optimization_opportunities"].append(
                    {
                        "section_id": section.section_id,
        ...
        report["top_performing_sections"] = [
            {
                ...
                "performance_score": round(
                    s.current_utilization * s.foot_traffic_multiplier, 3
                ),
...
def optimize_store_sections(
    ...
    return self.section_optimizer.optimize_store_sections(
        target_sections, optimization_criteria
    )
