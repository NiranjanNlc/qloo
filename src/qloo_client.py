...
class QlooClient:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None, 
                 max_retries: int = 3, initial_delay: float = 1.0, max_delay: float = 60.0,
                 timeout: float = 10.0):
        """
        ...
        self.base_url = base_url or os.getenv("QLOO_BASE_URL", "https://hackathon.api.qloo.com")
        ...
        if not self.api_key:
            raise ValueError("Qloo API key is required. Set QLOO_API_KEY environment variable or pass api_key parameter.")
    ...
    def _should_retry(self, exception: Exception, attempt: int) -> bool:
        ...
        if isinstance(exception, requests.HTTPError):
            ...
            if exception.response.status_code >= 500 or exception.response.status_code == 429:
                return True
                
        return False

    def _make_request_with_retry(self, endpoint: str, method: str = "GET", 
            ...
            try:
                ...
                if not self._should_retry(e, attempt) or is_last_attempt:
                    ...
                    if isinstance(e, requests.Timeout):
                        raise QlooTimeoutError(f"Request timed out after {self.max_retries + 1} attempts")
                    elif isinstance(e, requests.HTTPError):
                        raise QlooAPIError(f"API error {e.response.status_code}: {e.response.text}")
                    else:
                        raise QlooAPIError(f"Request failed: {str(e)}")
                
                # Wait before retrying
                delay = self._exponential_backoff_delay(attempt)
                print(f"Request failed (attempt {attempt + 1}), retrying in {delay:.2f}s: {str(e)}")
                time.sleep(delay)
        
        # Should never reach here, but just in case
        raise QlooAPIError("Unexpected error: exhausted all retries")
    
    def get_product_recommendations(self, product_id: str, limit: int = 10, recommendation_type: str = "product") -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            response = self._make_request_with_retry("search", method="GET", params=params)
    ...
    def get_product_associations(self, product_ids: List[str], association_type: str = "product") -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            for product_id in product_ids:
                ...
                associations[product_id] = [
                    {
                        "associated_product_id": result.get("id", f"related_{product_id}"),
    ...
    def get_category_insights(self, category: str, insight_type: str = "category") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "total_products": 150,
                "top_products": [
                    {"id": f"{category}_product_{i}", "popularity_score": 0.9 - (i * 0.1)}
                    for i in range(1, 6)
                ],
                "seasonal_trends": {
                    "spring": 0.8,
