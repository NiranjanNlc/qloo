...
class ConfigurationManager:
    ...
    def get_heuristic_config(self, heuristic_name: str) -> Dict[str, Any]:
        ...
        return self.config.get('optimization', {}).get('heuristics', {}).get(heuristic_name, {})
...
class HeuristicOptimizer:
    ...
    def calculate_heuristic_scores(self, 
                                 target_products: Optional[List[int]] = None) -> List[HeuristicScore]:
        """
        ...
        for product_id in target_products:
            ...
            crowding_penalty = self._calculate_crowding_penalty(product_id, current_location)
            adjacency_reward = self._calculate_adjacency_reward(product_id, current_location)
            popularity_bonus = self._calculate_popularity_bonus(product_id, current_location)
            cross_selling_bonus = self._calculate_cross_selling_bonus(product_id, current_location)
            
            # Combine components into final score
            final_score = (base_score - crowding_penalty + adjacency_reward + 
                         popularity_bonus + cross_selling_bonus)
    ...
    def _calculate_base_score(self, product_id: int, location: ProductLocation) -> float:
    ...
    def _calculate_crowding_penalty(self, product_id: int, location: ProductLocation) -> float:
        ...
        logger.debug(f"Crowding penalty for product {product_id}: {penalty:.3f} "
                    f"(utilization: {utilization:.2f})")
        
        return penalty
    
    def _calculate_adjacency_reward(self, product_id: int, location: ProductLocation) -> float:
        ...
        for neighbor_id in neighbors:
            ...
            association_reward = (base_reward * association_factor * 
                                distance_factor * category_factor)
    ...
    def _calculate_popularity_bonus(self, product_id: int, location: ProductLocation) -> float:
        ...
        logger.debug(f"Popularity bonus for product {product_id}: {bonus:.3f} "
                    f"(popularity: {popularity:.2f})")
        return bonus
    
    def _calculate_cross_selling_bonus(self, product_id: int, location: ProductLocation) -> float:
        ...
        for rule in self.layout_optimizer.association_engine.association_rules:
            ...
            for other_product in (antecedent + consequent):
                if other_product != product_id and other_product in section_products:
                    ...
                    rule_bonus = (rule['confidence'] * rule['lift'] * 
                                complementary_bonus * basket_affinity_weight)
                    bonus += rule_bonus
        
        logger.debug(f"Cross-selling bonus for product {product_id}: {bonus:.3f}")
        return bonus
    
    def generate_placement_suggestions(self, 
                                     min_improvement: Optional[float] = None) -> List[PlacementSuggestion]:
        """
        ...
        if min_improvement is None:
            min_improvement = self.config_manager.get_performance_config().get(
                'thresholds', {}
            ...
            if best_alternative:
                ...
                if improvement >= min_improvement:
                    suggestion = PlacementSuggestion(
                        product_id=product_id,
                        current_section=self._get_section_for_location(current_location),
                        suggested_section=alt_section,
                        confidence_score=min(improvement / 0.5, 1.0),  # Normalize to 0-1
    ...
    def _find_best_alternative_placement(self, 
                                       product_id: int, 
                                       current_location: ProductLocation,
                                       current_score: float) -> Optional[Tuple[str, float, str]]:
        ...
        if hasattr(self.layout_optimizer, 'section_optimizer'):
            ...
            for section_id, section in sections.items():
                ...
                hyp_score = self._calculate_hypothetical_score(product_id, hypothetical_location)
    ...
    def _calculate_hypothetical_score(self, product_id: int, location: ProductLocation) -> float:
    ...
    def _get_section_for_location(self, location: ProductLocation) -> str:
        ...
        if hasattr(self.layout_optimizer, 'section_optimizer'):
            for section_id, section in self.layout_optimizer.section_optimizer.store_sections.items():
                if (section.parent_zone == location.zone and
    ...
    def _get_section_utilization(self, section_id: str) -> float:
        ...
        if hasattr(self.layout_optimizer, 'section_optimizer'):
            section = self.layout_optimizer.section_optimizer.store_sections.get(section_id)
            if section:
    ...
    def _get_products_in_section(self, section_id: str) -> List[int]:
        ...
        if hasattr(self.layout_optimizer, 'section_optimizer'):
            section = self.layout_optimizer.section_optimizer.store_sections.get(section_id)
            if section:
                return self.layout_optimizer.section_optimizer._get_products_in_section(section)
        return []
    ...
    def _get_product_popularity(self, product_id: int) -> float:
        ...
        if (self.layout_optimizer.association_engine and 
            self.layout_optimizer.association_engine.is_trained):
            
            # Calculate popularity from frequent itemsets
            for itemset_size, itemsets in self.layout_optimizer.association_engine.frequent_itemsets.items():
                if itemset_size == 1:
                    for itemset, count in itemsets.items():
                        if list(itemset)[0] == product_id:
                            popularity = count / len(self.layout_optimizer.association_engine.transactions)
                            break
    ...
    def _generate_placement_reasoning(self, section: 'StoreSection', improvement: float) -> str:
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'high_impact_suggestions': len([s for s in suggestions if s.expected_improvement > 0.2]),
            ...
            'configuration_status': 'loaded' if self.config_manager.config else 'default',
